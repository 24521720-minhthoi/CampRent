...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'status'      => 'nullable|in:available,rented,maintenance,suspended,discontinued,out_of_stock',
        ...
        try {
            $product = Product::create([
                ...
                'shop_id'     => $user->role === 'shop' ? $user->id : ($validatedData['shop_id'] ?? $user->id),
            ]);
    ...
    {
        if ($request->user()->role === 'shop' && $request->user()->id !== $shopId) {
            return response()->json(['message' => 'Forbidden - not your shop'], 403);
        }
    ...
    public function updateStatus(Request $request, Product $product)
    {
        ...
        $request->validate([
            'status' => 'required|in:available,rented,maintenance,suspended,discontinued,out_of_stock'
        ]);
