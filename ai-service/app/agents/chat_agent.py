...
class ChatAgent:
    ...
    SYSTEM_PROMPT = """Bạn là trợ lý ảo của CampRent — nền tảng cho thuê đồ cắm trại và outdoor hàng đầu Việt Nam.
Nhiệm vụ của bạn là tư vấn, hỗ trợ khách hàng tìm đồ cắm trại, phượt, leo núi, dã ngoại phù hợp và trả lời các câu hỏi về đơn hàng thuê.

Khi trả lời:
- Sử dụng tiếng Việt tự nhiên, thân thiện, vui vẻ với phong cách năng động outdoor
- Trích dẫn thông tin sản phẩm cụ thể (tên, giá thuê/ngày, danh mục)
- Nếu không tìm thấy sản phẩm phù hợp, gợi ý khách hàng mô tả chi tiết hơn nhu cầu chuyến đi
- Format giá tiền theo định dạng Việt Nam (ví dụ: 500.000₫/ngày)
- Khi nói về tồn kho, nếu stock > 0 là "còn hàng", nếu = 0 là "hết hàng"
- Luôn nhắc khách hàng về tiền đặt cọc nếu sản phẩm có yêu cầu đặt cọc
- Có thể gợi ý combo đồ cắm trại phù hợp cho từng loại chuyến đi
...
"""
    ...
    def _build_order_history_context(self, user_id: int) -> str:
        ...
        if not orders:
            return "Khách hàng chưa có đơn thuê nào."
        
        context_parts = ["Lịch sử đơn thuê gần đây của khách hàng:"]
        for order in orders:
            items_text = ", ".join([
    ...
    def _build_order_status_context(self, order_id: int, user_id: int) -> str:
        ...
        if not order:
            return f"Không tìm thấy đơn thuê #{order_id} trong hệ thống của khách hàng."
        
        return (
            f"Thông tin đơn thuê #{order['id']}:\n"
    ...
    def _build_best_sellers_context(self) -> str:
        ...
        context_parts = ["Top sản phẩm outdoor được thuê nhiều nhất:"]
        for i, product in enumerate(products, 1):
            context_parts.append(
                f"{i}. {product['name']} - {self._format_price(float(product['price']))}/ngày "
    ...
    def _build_price_query_context(self, order: str = "DESC", limit: int = 5) -> tuple[str, list[dict]]:
        ...
        context_parts = [f"Top {limit} sản phẩm outdoor có giá thuê {label}:"]
        sources = []
        
        for i, product in enumerate(products, 1):
            price = float(product['price']) if product['price'] else 0
            context_parts.append(
                f"{i}. {product['name']} - {self._format_price(price)}/ngày | "
                f"Danh mục: {product.get('category_name', 'N/A')} | "
                f"Còn: {product['stock']} sản phẩm"
            )
            sources.append({
    ...
    def _build_product_search_context(self, query: str) -> tuple[str, list[dict]]:
        ...
        if not similar_products:
            return "Không tìm thấy sản phẩm outdoor liên quan trong hệ thống.", []
        
        context_parts = ["Các sản phẩm outdoor phù hợp với yêu cầu:"]
        for product in similar_products:
            context_parts.append(
                f"- {product['name']} | Giá: {self._format_price(product['price'])}/ngày | "
